...
async def adventure(context, *args, **kwargs):
    ...
    if (len(cleaned_args) < 1) or (cleaned_args[0] == 'help'):
        adventure_help_msg = "I can generate an adventure for you! Specify `{cp}adventure` with the type of story you'd like!\n" + \
                             "*Core, Extended, Navis, Undernet, NetAgents, Chaos*"
...
# -----------------------------------------------------------------------
# ADVENTURE HEADERS
# THE DATASET IS MAINTAINED HERE:
# https://docs.google.com/spreadsheets/d/1Jxff9q-m69M6NQYuwghGf6uvmBwvaYHImusUNVOe8pw/edit#gid=0
...
# These three work together (Except core rulebook doesn't care about ClassHeader)
...
# Conflict generators. Headers for conflicts and vulnerabilities primarily exist for homebrew tables.
    ...
    vulnheaddf_sub = adventure_df[adventure_df["Type"] == "VulnHeader"]
    row_num = random.randint(1, vulnheaddf_sub.shape[0]) - 1
    sort_vulnhead = [vulnheaddf_sub.iloc[row_num]["Sort"]]
    define_vulnhead = [vulnheaddf_sub.iloc[row_num]["Definition"]]
    vuln_header = [vulnheaddf_sub.iloc[row_num]["Result"]]
...
# Extended generator tables.
    ...
    atmospheredf_sub = adventure_df[adventure_df["Type"] == "Atmosphere"]
    row_num = random.randint(1, atmospheredf_sub.shape[0]) - 1
    atmosphere = [atmospheredf_sub.iloc[row_num]["Result"]]
...
# Maybe to-do navi names too?
    ...
    npclastdf_sub = adventure_df[adventure_df["Type"] == "NPCLastName"]
    row_num = random.randint(1, npclastdf_sub.shape[0]) - 1
    npc_lastname = [npclastdf_sub.iloc[row_num]["Result"]]
...
# -----------------------------------------------------------------------
    ...
    if (args[0] == 'core'):
        print(arg)
        print(*sort_advheader)
        print(sort_advheader[0].lower())
        while sort_advheader[0].lower() != 'core':
            row_num = random.randint(1, advheaddf_sub.shape[0]) - 1
            sort_advheader = [advheaddf_sub.iloc[row_num]["Sort"]]
            define_advheader = [advheaddf_sub.iloc[row_num]["Definition"]]
            adv_header = [advheaddf_sub.iloc[row_num]["Result"]]
            print(*adv_header, row_num, *sort_advheader)
            if sort_advheader[0].lower() == 'core':
                print("Found suitable replacement:", *sort_advheader)
                break
        while sort_headresult[0].lower() != 'core' or define_headresult[0] != define_advheader[0]:
            ...
            define_headresult = [headresultdf_sub.iloc[row_num]["Definition"]]
            header_result = [headresultdf_sub.iloc[row_num]["Result"]]
            if sort_headresult[0].lower() == 'core' and define_headresult[0] == define_advheader[0]:
                print("Second header matched successfully")
                break
        while sort_conflictresult[0].lower() != 'core':
            ...
            define_conflictresult = [conflictresultdf_sub.iloc[row_num]["Definition"]]
            conflict_result = [conflictresultdf_sub.iloc[row_num]["Result"]]
            if sort_conflictresult[0].lower() == 'core':
                print("Third header matched successfully")
                break
        while sort_npcfirst[0].lower() != 'core':
            row_num = random.randint(1, npcfirstdf_sub.shape[0]) - 1
            sort_npcfirst = [npcfirstdf_sub.iloc[row_num]["Sort"]]
            npc_firstname = [npcfirstdf_sub.iloc[row_num]["Result"]]
            if sort_npcfirst[0].lower() == 'core':
                print("Fourth header matched successfully")
                break
        while sort_npcpersonality[0].lower() != 'core':
            row_num = random.randint(1, npcpersonalitydf_sub.shape[0]) - 1
            sort_npcpersonality = [npcpersonalitydf_sub.iloc[row_num]["Sort"]]
            npc_personality = [npcpersonalitydf_sub.iloc[row_num]["Result"]]
            if sort_npcpersonality[0].lower() == 'core':
                print("Fifth header matched successfully")
                break
        while sort_npcoccupation[0].lower() != 'core':
            row_num = random.randint(1, npcoccupationdf_sub.shape[0]) - 1
            sort_npcoccupation = [npcoccupationdf_sub.iloc[row_num]["Sort"]]
            npc_occupation = [npcoccupationdf_sub.iloc[row_num]["Result"]]
            if sort_npcoccupation[0].lower() == 'core':
                print("Sixth header matched successfully")
                break
        while sort_npcfeature[0].lower() != 'core':
            row_num = random.randint(1, npcfeaturedf_sub.shape[0]) - 1
            sort_npcfeature = [npcfeaturedf_sub.iloc[row_num]["Sort"]]
            npc_feature = [npcfeaturedf_sub.iloc[row_num]["Result"]]
            if sort_npcfeature[0].lower() == 'core':
                print("Seventh header matched successfully")
                break
        while sort_vulnres[0].lower() != 'core':
            ...
            define_vulnres = [vulnresdf_sub.iloc[row_num]["Definition"]]
            vuln_result = [vulnresdf_sub.iloc[row_num]["Result"]]
            if sort_vulnres[0].lower() == 'core':
                print("Eighth header matched successfully")
                break
    ...
    if (args[0] == 'extended'):
        # class_header -> adv_header -> header_result
        while define_headresult[0] != define_advheader[0]:
            row_num = random.randint(1, headresultdf_sub.shape[0]) - 1
            sort_headresult = [headresultdf_sub.iloc[row_num]["Sort"]]
            define_headresult = [headresultdf_sub.iloc[row_num]["Definition"]]
            header_result = [headresultdf_sub.iloc[row_num]["Result"]]
            if define_headresult[0] == define_advheader[0]:
                print("Second header matched successfully")
                break
        generated_msg = "The adventure starts with {} {} {} " + \
                        "But {} {} {} {}\n" + \
                        "The tone of the adventure is {}, and the primary conflict is {} " + \
                        "**{} {}** is {} {}, notable for {}.\n" + \
        ...
        return await koduck.sendmessage(context["message"],
                                        sendcontent=generated_msg.format(*class_header, *adv_header, *header_result,
                                                                         *conflict_header, *conflict_result, *vuln_header,
                                                                         *vuln_result, *atmosphere, *conflict_type,
                                                                         *npc_firstname, *npc_lastname, *npc_personality,
                                                                         *npc_occupation, *npc_feature, *class_header,
                                                                         *navi_personality, *navi_element, *navi_hostility))
    ...
    if (args[0] == 'chaos'):
        ...
        return await koduck.sendmessage(context["message"],
                                        sendcontent="Please specify either Core, Extended, Navis, Undernet, NetAgents, or Chaos.")
